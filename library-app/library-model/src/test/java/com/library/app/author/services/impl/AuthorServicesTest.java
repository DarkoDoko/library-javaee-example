...
public class AuthorServicesTest {
	...
	public void initializeTestCase() {
		...
		((AuthorServicesImpl) servicesUnderTest).repository = repository;
		((AuthorServicesImpl) servicesUnderTest).validator = validator;
	}
	...
	public void addValidAuthor() {
		when(repository.add(robertMartin())).thenReturn(authorWithId(robertMartin(), 1L));
	...
	public void updateValidAuthor() throws Exception {
		...
		verify(repository).update(authorToUpdate);
	}
	...
	public void findAuthorById() throws AuthorNotFoundException {
		when(repository.findById(1L)).thenReturn(authorWithId(robertMartin(), 1L));
	...
	public void findAuthorByFilter() {
		final PaginatedData<Author> authors = new PaginatedData<Author>(1, Arrays.asList(authorWithId(robertMartin(),
				1L)));
		when(repository.findByFilter((AuthorFilter) anyObject())).thenReturn(authors);
