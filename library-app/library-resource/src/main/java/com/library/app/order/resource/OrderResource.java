...
public class OrderResource {
    ...
    @Path("/{id}/status")
    @PermitAll
    public Response addStatus(@PathParam("id")Long id, String body) {
        logger.debug("Adding the new status {} for order {}", body, id);

		OrderStatus status = getStatusFromJson(body);
		...
		} catch (OrderStatusCannotBeChangedException e) {
    ...
    public Response findByFilter(){
		OrderFilter orderFilter = new OrderFilterExtractorFromUrl(uriInfo).getFilter();
		logger.debug("Finding orders using filter: {}", orderFilter);

		PaginatedData<Order> orders = orderServices.findByFilter(orderFilter);

		logger.debug("Found {} orders", orders.getNumberOfRows());

		JsonElement jsonWithPagingAndEntries = JsonUtils.getJsonElementWithPagingAndEntries(orders,
				orderJsonConverter);
		return Response.status(HttpCode.OK.getCode()).entity(JsonWriter.writeToString(jsonWithPagingAndEntries))
				.build();
    }

    private OrderStatus getStatusFromJson(String body) {
