...
public class OrderResourceIntTest {
    ...
    public void initTestCase() {
        ...
        resourceClient.resourcePath("DB/books").postWithContent("");
	...
	public void addValidOrderAndFindIt() {
		Long orderId = addOrderAndGetId(normalizeDependenciesWithRest(orderReserved()));
	...
	public void addOrderWithInexistentBook() {
		Order order = orderReserved();
		order.getItems().iterator().next().getBook().setId(999L);

		Response response = resourceClient.user(johnDoe()).resourcePath(PATH_RESOURCE)
	...
	public void addOrderAsAdmin() {
		Order order = normalizeDependenciesWithRest(orderReserved());

		Response response = resourceClient.user(admin()).resourcePath(PATH_RESOURCE)
				.postWithContent(getJsonForOrder(order));
	...
	public void addStatusDeliveredAsNotEmployee() {
		...
		Response response = resourceClient.user(johnDoe())
	...
	public void addStatusDeliveredAsEmployee() {
		...
		Response response = resourceClient.user(admin()).resourcePath(PATH_RESOURCE + "/" + orderId + "/status")
				.postWithContent(getStatusAsJson(OrderStatus.DELIVERED));
    ...
    private void assertResponseContainsTheOrdersStatus(Response response, int expectedTotalRecords, 
        OrderStatus... expectedOrdersStatus) {

		JsonArray ordersList = IntTestUtils.assertJsonHasTheNumberOfElementsAndReturnTheEntries(response,
	...
	private Order normalizeDependenciesWithRest(Order order) {
